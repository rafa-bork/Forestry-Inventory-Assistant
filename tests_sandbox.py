...
def read_data():
    ...
    try:
        ...
        objects = create_tree_objects(df)
        return objects
...
class Tree:
    ...
    def is_duplicate_tree_ID(tree_ID):
       f"Tree(tree_ID={self.tree_ID}, species={self.species}, dbh={self.dbh}, height={self.height}, cod_status={self.cod_status})"
    ...
    def __repr__(self):
        return f"The Tree {self.tree_ID} ({self.species}) has a diameter of {self.dbh} cm and a height of ({self.height}) (cod_status={self.cod_status})"      self.set_species(species)
        self.set_dbh(dbh)
...
def create_tree_objects(df):
    ...
    print("Data imported successfully.")         tree.set_attributes(species, dbh, height, cod_status)
